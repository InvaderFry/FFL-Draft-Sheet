...
import { stubSheet, stubDraft, stubSleeperDraft, fixture } from './helpers'
...
test.describe('live ESPN draft sync', () => {
  test('synced picks cross players off the board', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /sync live draft/i }).click()
...
test.describe('live Sleeper draft sync', () => {
  test('synced picks cross players off the board', async ({ page }) => {
    await stubSheet(page)
    await stubSleeperDraft(page)
    await page.goto('/')
    ...
    // Open the sync form, switch to the Sleeper tab, enter a draft id, connect.
    await page.getByRole('button', { name: /sync live draft/i }).click()
    await page.getByRole('tab', { name: /sleeper/i }).click()
    await page.getByPlaceholder(/1109123456789012345/).fill('123456')
    await page.getByRole('button', { name: /^Connect$/ }).click()
